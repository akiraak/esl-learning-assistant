...
## 実装時の変更点（2026-07-03 実装完了）

- **料金は per-image 固定額ではなくトークン単価制だった**: GPT Image 2（モデルID
  `gpt-image-2`）の公式料金はテキスト入力 $5.00 / 画像出力 $30.00（per 1M tokens）で、
  Images API レスポンスの `usage`（input_tokens / output_tokens）から既存の
  `estimateCostUsd` でそのまま算出できる。画像専用の単価テーブルは
  `DEFAULT_IMAGE_PRICING`（pricing.ts、自動更新対象外の手動固定値）として追加した
- `word_illustrations` テーブルにはトークン数記録のため `input_tokens` / `output_tokens`
  列を追加した（tts_audio と同じ構成）
- 生成パラメータは 1024x1024 / quality=low で固定（約 $0.006/枚。当初 medium で実装したが
  コスト優先で最低品質に変更）
- iOS のイラストセクションは自動取得ではなく生成ボタン方式にした
  （TTSButton と同じ状態遷移。コスト発生操作を明示的にするため）
- 管理画面の再生成は保存済みプロンプトを使い回す（単語情報が更新されていても
  プロンプトは据え置きの割り切り）
- 実画像の生成は `OPENAI_API_KEY` 未設定のため未確認（設定後にユーザーが確認する）。
  バリデーション・401/400/500系・キャッシュヒット（正規化含む）・管理画面表示は確認済み
