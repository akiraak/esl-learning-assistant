...
- **Phase 3: YouTube の横断一覧**（同様に YouTubeLink 集約、セグメントに YouTube 追加）
  - `YouTubeLibraryView` を新設: `@Query` で全 YouTubeLink を追加日降順に一覧
    （行タップで `YouTubeDetailView` へ push、スワイプ削除は LessonsView と同じく確認なし・
    詳細画面側の削除には既存の確認ダイアログあり）
  - `YouTubeRow` に `showsLesson` を追加（`PhotoRow` と同様にレッスン名サブタイトルを表示）
  - YouTube はレッスン必須（to-one）のため、「+」は `YouTubeAddView` を CaptureView と同様に拡張:
    固定レッスン無しで開いた場合はフォーム内のレッスン選択（既定は最新レッスン）で追加先を選ぶ。
    レッスンが無ければ案内を表示して入力フォームを出さない
  - セグメント構成は Photos | Audio | YouTube | Docs（4個になるため Documents は Docs に短縮）
