...
## 追加検証: クライアント側キャッシュと「再生成の反映」（2026-07-06）
- iOS にも永続ローカルキャッシュ `TTSAudioStore`（Application Support/tts/）があり、
  キーはサーバと同じ `sha256("model|text")`（`TTSAudioStore.swift:15-17`）。
- 全再生経路が「ローカルにあればサーバに問い合わせず使う」実装:
  - 発音ボタン `TTSButton.swift:31`、クイズ `QuizAudioDownloader.swift:51`、`ReviewSessionView.swift:798`。
- → 管理画面で再生成しても model/text は不変（"lazy"）＝キー不変のため、**既にキャッシュ済みの端末は
