...
- [x] 2026-08-03 相談チャットに「全ページを含める」スイッチを付けた
      ...
      「全体の流れは自然？」が聞けなかった。AI 欄のヘッダ（モデル名の行）にチェックボックスを置き、
      入れると同じ作文の全ページを添える。状態はペイン幅と同じく localStorage
      （`composition.chatAllPages`）に覚える。プロンプトは `compositionChat.ts` の
      `buildChatPagesText()` が組み、複数ページのときはタブ名を見出しにして並べ、いま開いている
      ページに `← いま開いているページ` の印と前置きを付ける（指示語が開いているページに向くように）。
      1 ページあたりは従来どおり 5000 字で切り、合計 `CHAT_ALL_PAGES_MAX_LENGTH = 20000` 字を
      超える分はページ単位で落として「省略した」ことをプロンプトに明記する（開いているページは
      末尾にあっても落とさない）。タイトル生成と選択翻訳の渡し方は変えていない。

- [x] 2026-08-03 執筆画面にワード数を表示するようにした
      [plan](docs/plans/archive/writing-word-count.md)
      ツールバーの右上に `123 words ・ 保存済み` と並べて出す。普段は見ているページ（タブ）1枚の
      語数で、英文を選んでいる間は `選択 10 words` とその範囲の語数に切り替わる。数え方は
      `composition.ts` の `WORD_PATTERN_SOURCE` に置き、サーバと画面で同じ正規表現を共有する
      （`don't` / `well-known` は 1 語、記号だけのかたまりは数えない）。初期値はサーバが数えて
      HTML に埋めるので開いた直後にちらつかない。更新は本文でも選択でも必ず通る `renderMarks()`
      の末尾で行う。あわせて、ページを切り替えても前のページの選択範囲が残っていた点を直した。
...
- [x] 2026-08-02 執筆画面のタイトルを本文の紙から分離して上に置いた
      ...
      左右の余白 `--pad-x` は `.sheet` から `.paper-pane` に移し、タイトルと本文の書き出しの
      左端が揃うようにしてある。罫線に載せる必要がなくなったので、タイトル行の高さの
      行送り固定はやめ、字を 26px にしてホバー / フォーカス時だけ細い下線を出す。
      狭い画面（≤720px）は紙と同じく影なし、印刷では背景・影とも落とす。
...
- [x] 2026-08-01 管理画面の作文でスペルミスを強調表示
      ...
      判定は `typo-js`（hunspell 互換・en_US 同梱）による辞書ベースの自前実装で AI は呼ばない
      （`/admin/usage` にも計上しない）。判定はサーバ（`spellcheck.ts` の `findMisspellings`）、
      描画はクライアント。辞書 557KB をブラウザへ配らずに済み、判定が純粋関数なので
      `node --test` で検証できる。トークナイズは「空白で切る → 前後の記号を落とす → 判定対象を
      絞る → ハイフンで分ける」の4段で、`doesn't` は1語のまま・`well-known` は片ごと・
      数字/略語/URL/メール/日本語はスキップする（辞書に無く必ず誤検出になるため）。
      強調は textarea の背後に**同じ字組みの下敷き**（`.paper-backdrop`）を敷き、該当語だけ
      `<mark>` で包む方式。字組みに関わる CSS は `.paper, .paper-backdrop` の1か所で両方に当てる
      （ここがズレると赤線が字の下から外れる。罫線と本文を揃えたときと同じ地雷）。標準の
      赤い波線と二重にならないよう `spellcheck="false"` にした。初回はサーバ側判定を
      `<script>` に埋め、以後は入力停止 600ms で `POST /writing/:id/spellcheck`。返答待ちの間に
      打鍵が進んでいたらオフセットが食い違うので、送った本文と一致するときだけ反映する。
      通信失敗時は直前の結果を残して黙って諦める（書く手を止めない）。打鍵の直後だけカーソル上の
      語を伏せ、打ち終えた瞬間に赤くなる。修正候補は `suggest()` が1語1.2秒かかる（`check()` は
      20,000回で1ms）ため一括では返さず、赤い語にカーソルを置いた時にその語だけ問い合わせて
      ポップオーバーに出す。選ぶと本文を置換して保存し、「辞書に追加」（`spell_ignored_words`）で
      固有名詞を以後どの作文でも赤くしない。例外語は単語帳の登録語（`words`）との和集合。
      既知の限界として英国綴り（`colour`）は赤くなる（en_US 辞書のため）。
