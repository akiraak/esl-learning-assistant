...
/// レッスン詳細のコンテンツ欄で写真・音声・文書・YouTube を1つのリストに束ねるためのビュー層の型。
/// モデルは統合せず（`Photo`/`AudioClip`/`Document`/`YouTubeLink` はそのまま）、表示時だけ共通の
...
enum LessonContentItem: Identifiable {
    ...
    /// 時系列マージ用の共通タイムスタンプ（写真=撮影日 / 音声=取込日 / 文書=取込日 / YouTube=追加日）。
    var sortDate: Date {
        switch self {
        case .photo(let photo): return photo.capturedAt
        case .audio(let clip): return clip.importedAt
        case .document(let document): return document.importedAt
