...
// 穴埋め形式（tc3/tc6/vtt1）の空欄検証（docs/plans/vtt1-blank-validation.md）。
// AI がまれに空欄化に失敗した完全文を返す（答えが本文に露出）ため、空欄なしを捨てる。

test("validateAndConvert: 空欄なしの vtt1 は捨てられる（答え露出の防止）", () => {
  const question = validateAndConvert(
    {
      format: "vtt1",
      variantIndex: 0,
      instruction: "Listen and type the missing word.",
      // 空欄化に失敗し、audioText と同じ完全文になった不良ケース
      displayText: "They decided to consolidate the two teams.",
      audioText: "They decided to consolidate the two teams.",
      answerType: "typing",
      options: null,
      correctIndex: null,
      acceptedAnswers: ["consolidate"],
    },
    "consolidate",
    specById("vtt1")
  );
...
test("validateAndConvert: 空欄ありの vtt1 は受理される", () => {
  const question = validateAndConvert(
    {
      format: "vtt1",
      variantIndex: 0,
      instruction: "Listen and type the missing word.",
      displayText: "They decided to _____ the two teams.",
      audioText: "They decided to consolidate the two teams.",
      answerType: "typing",
      options: null,
      correctIndex: null,
  ...
  assert.ok(question);
  assert.equal(question.displayText, "They decided to _____ the two teams.");
  assert.equal(question.audioText, "They decided to consolidate the two teams.");
});

test("validateAndConvert: 空欄なしの tc3（4択穴埋め）も捨てられる", () => {
  const question = validateAndConvert(
    {
      format: "tc3",
      variantIndex: 0,
      instruction: "Choose the word that fits the blank.",
      displayText: "She runs every morning.",
      audioText: null,
      answerType: "choices",
      options: ["runs", "eats", "reads", "sings"],
      correctIndex: 0,
